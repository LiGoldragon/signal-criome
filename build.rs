...
const MODULE_PATH: &[&str] = &["signal_criome", "lib"];
...
impl SchemaBuild {
    ...
    fn run(&self) {
        ...
        let source =
            fs::read_to_string(&source_path).expect("read ordinary Criome Interface source");
        ...
        .expect("assemble authority-approved ordinary Criome Interface transaction");
        let rust = rust_logos();
        let type_paths = CriomeRustTypePaths::new();

        BootstrapInterfaceGeneration::new(&assembly, &rust, &type_paths, &source_path, &rust_path)
            .generate()
            .expect("project ordinary Criome Interface from the verified transaction")
            .write_or_check("SIGNAL_CRIOME_UPDATE_INTERFACE_ARTIFACTS")
            .expect("checked-in ordinary Criome Interface source and Rust projection are fresh");
...
fn bootstrap_catalog() -> BootstrapCatalog {
    ...
    .expect("Criome bootstrap catalog is complete")
}
